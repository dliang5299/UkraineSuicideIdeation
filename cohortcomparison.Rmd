...
cididata_pt2 <-
  ...
  select(sampleid, brom4_agecohort, brom3_agecohort, alfonso_cohort, SEX, io16, 
         de20, fnu1, fnu4, de32_1, de32_2, de32_other, de32_9, emp, empw, empst, emph, 
         empr, empoth,de34, pt6a, pt6b, de40, live_chernobyl, married, sc3, 
         mr16, mr18, mr22, sn1, sn2, sn6, sn7, SN1, SN2, SN6, SN7, cc1tz, cc2, 
         cn6, cn6a,ideate, plan, attempt, ch67, ch97, ch52, ch82, pt1,
...
svytable(~alfonso_cohort + married, design = SuicidePart2) %>%
  addmargins() %>%
  round()
svytable(~brom3_agecohort + married, design = SuicidePart2) %>% 
  addmargins() %>%
  round()
```
...
```{r}
svyglm(ideate~married, family = quasibinomial("logit"), design = SuicidePart2) %>%
  summary()
svyglm(plan~married, family = quasibinomial("logit"), design = SuicidePart2) %>%
  summary()
svyglm(attempt~married, family = quasibinomial("logit"), design = SuicidePart2) %>%
  summary()
```
...
```{r}
# Cancer present
svyglm(ideate~cc1tz, family = quasibinomial("logit"), design = SuicidePart2) %>%
  summary()
svyglm(plan~cc1tz, family = quasibinomial("logit"), design = SuicidePart2) %>%
  summary()
svyglm(attempt~cc1tz, family = quasibinomial("logit"), design = SuicidePart2) %>%
  summary()
```
...
```{r}
# financial status
svyglm(ideate~fnu4, family = quasibinomial("logit"), design = SuicidePart2) %>%
  summary()
svyglm(plan~fnu4, family = quasibinomial("logit"), design = SuicidePart2) %>%
  summary()
svyglm(attempt~fnu4, family = quasibinomial("logit"), design = SuicidePart2) %>%
  summary()
```

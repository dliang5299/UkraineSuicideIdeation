...
```{r}
ideate_glm <-
  svyglm(ideate~SEX+io16+marital_status+SN7+Malad+Maltreat+ChdIll+aff+ied+any_cc,
...
```{r}
plan_glm <-
  svyglm(plan~SEX+emp+ChdIll+alc+aff,
...
```{r}
attempt_glm <-
  svyglm(attempt~brom3_agecohort+io16+marital_status+alc+aff+ied,
         family = quasibinomial("logit"), design = ideatedesign)
summary(attempt_glm)
```
